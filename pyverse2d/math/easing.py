...
def easing(func: EasingFunc) -> EasingFunc:
...
def linear(t: float) -> float:
    """Progression linéaire classique"""
    return t

# ======================================== QUAD ========================================
@easing
def ease_in_quad(t: float) -> float:
    """Démarre doucement puis accélère avec une accélération proportionnelle à t"""
    return t ** 2
...
def ease_in_out_expo(t: float) -> float:
    """Quasi immobile au départ, vitesse maximale au milieu, quasi immobile à l'arrivée"""
...
# ======================================== BACK ========================================
_BACK_C1: float = 1.70158
_BACK_C2: float = _BACK_C1 * 1.525
_BACK_C3: float = _BACK_C1 + 1

@easing
def ease_in_back(t: float) -> float:
    """Recule légèrement avant de partir en avant"""
...
def ease_in_out_back(t: float) -> float:
    """Recule au départ et dépasse à l'arrivée"""
    if t < 0.5:
...
def ease_in_elastic(t: float) -> float:
    """Oscille autour de 0 avec une amplitude croissante avant de partir brusquement, comme un ressort qu'on comprime avant de lâcher"""
    return np.where(t == 0, 0.0, np.where(t == 1, 1.0, -(2 ** (10 * t - 10)) * np.sin((t * 10 - 10.75) * _ELASTIC_C4)))

@easing
def ease_out_elastic(t: float) -> float:
    """Dépasse la cible puis oscille autour de 1 avec une amplitude décroissante, comme un ressort qui se stabilise"""
    return np.where(t == 0, 0.0, np.where(t == 1, 1.0, 2 ** (-10 * t) * np.sin((t * 10 - 0.75) * _ELASTIC_C4) + 1))

@easing
def ease_in_out_elastic(t: float) -> float:
    """Oscillations aux deux extrémités"""
